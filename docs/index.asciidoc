...
| codec_metadata    | Information about how the codec transformed a sequence of bytes into
                      this Event, such as _which_ codec was used. If no <<codec>> is explicitly
                      specified, _excluding_ `codec_metadata` from `enrich` will prevent the
                      default codec itself from adding metadata such as `[event][original]`.
...
| default           | _alias_ resolving to _at least_ `[codec_metadata, source_metadata]`
...
| none              | _alias_ to _exclude_ all available enrichments
|=======================================================================
  ** Default value is `default`, which is an alias for for `[codec_metadata, source_metadata]` that may be extended in future versions of this plugin to include additional by-default enrichments.


As an example, following configuration disables _all_ enrichments:
