...
| <<plugins-{type}s-{plugin}-enrich>> |<<array,array>>|No
|=======================================================================
...
By default, the plugin initializes ECS-disabled plain codec, source and codec metadata such as source host or `[event][original]` will not be added.
`enrich` option helps to manage which metadata to be included. Available options are:

[cols="<,<"]
|=======================================================================
|Option |Description

|`none`                  |Not to include any data
|`default` alias | `codec_metadata` and `codec_metadata` will be included
|`all` alias | All `codec_metadata`, `codec_metadata` and `ssl_peer_metadata` will be included
|`source_metadata` | Source metadata such as host name will be included
|`codec_metadata` | Codec metadata such as `tag`, `[event][original]` will be included
|`ssl_peer_metadata` | SSL peer metadata will be included
|=======================================================================

As an example, following configuration enables source and metadata with ECS enabled plain codec:
```
input {
  beats {
    port => 5044
